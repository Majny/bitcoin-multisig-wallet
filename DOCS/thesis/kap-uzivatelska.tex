...
\caption{PSBT workflow. Zleva: seznam PSBT transakcí; detail PSBT
bezprostředně po vytvoření (0 ze~2 podpisů) s~tlačítkem \emph{Cancel
PSBT} pro stažení návrhu před podpisem; dialog signerů se stavy
Signed~/~Pending a~příznakem \emph{YOU}; PSBT s~dosaženým prahem
(2 ze~2) připravené k~broadcastu.}
...
UTXO zpět k~použití v~jiné transakci. Po broadcastu se tlačítko skrývá,

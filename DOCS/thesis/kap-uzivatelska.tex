...
PostgreSQL~\cite{PostgreSQL}, ve které jsou při prvním spuštění automaticky
vytvořeny tři oddělené databáze (\texttt{wallet\_auth},
\texttt{wallet\_registry}, \texttt{wallet\_psbt}) inicializačním skriptem
\texttt{init-db.sql}.
...
Mobilní aplikace je dostupná pouze ve formě zdrojového kódu v~adresáři
\texttt{wallet-frontend} a~je nutné si ji sestavit lokálně. Pro vývojové
i~uživatelské nasazení používáme stejný postup -- otevřeme adresář
v~Android Studiu, počkáme na dokončení Gradle synchronizace a~zvolíme
\emph{Run} pro instalaci a~spuštění na připojeném zařízení (případně
\emph{Build $\to$ Build APK} pro samostatný instalační soubor).

Před prvním sestavením je nutné v~souboru
\texttt{core/api/ApiConfig.kt} upravit konstantu \texttt{API\_GATEWAY\_BASE\_URL}
tak, aby odpovídala adrese, na které běží backend. Pro lokální testování,
kdy backend běží na desktopovém PC ve stejné síti, má hodnota typicky
podobu \texttt{http://192.168.X.Y:8080/api/v1}. Adresa je zapsaná
v~kódu záměrně, nikoli v~konfiguraci dostupné z~UI -- mobilní aplikace
předpokládá důvěryhodný backend, jehož adresa se v~čase nemění. Po úpravě
adresy je nutné aplikaci znovu sestavit.
...
Uživatel připojí Trezor přes USB k~mobilnímu zařízení a~stiskne tlačítko
\emph{Connect Trezor}. Aplikace přes deeplink přepne na Trezor
...
podle BIP-44~\cite{BIP44} -- prohledá prvních pět adres a~zjistí, zda na
nich proběhly nějaké transakce. Pro účty, na kterých discovery najde
historii, vytvoří záznam peněženky a~vrátí seznam aplikaci. Uživatel ze
...
\caption{Průběh přihlášení. Vlevo: přihlašovací obrazovka s~přepínačem
sítě. Uprostřed: výběr singlesig účtu -- backend nalezl čtyři účty
s~historií. Vpravo: hlavní obrazovka (dashboard) se zůstatkem v~BTC
a~CZK, seznamem transakcí a~tlačítky pro odeslání a~příjem.}
\label{fig:screen-onboarding}
\end{figure}

Tento postup probíhá pouze při prvním přihlášení nebo po vypršení tokenu;
v~běžném provozu aplikace přihlášení obnoví automaticky bez nutnosti
dalšího exportu z~Trezoru.
...
nebo příznak \emph{Pending} u~dosud nepotvrzených transakcí. V~dolní části
...
\caption{Příjem prostředků -- QR kód ve formátu BIP-21, adresa s~možností
kopírování a~tlačítko pro ověření adresy na displeji Trezoru.}
...
\caption{Odeslání transakce. Vlevo: formulář s~automatickým výběrem UTXO
a~třemi úrovněmi poplatku. Uprostřed: manuální režim s~počtem vybraných
UTXO a~polem pro vlastní sazbu v~sat/vB. Vpravo: potvrzení úspěšného
odeslání se souhrnem částky a~poplatku.}
...
\caption{Vlevo: coin control -- seznam UTXO s~hodnotou, adresou, datem
a~stavem potvrzení. Uprostřed: detail transakce v~hloubce 2\,/\,5
s~příznaky \emph{YOURS} a~\emph{FROM HERE}. Vpravo: nastavení --
přepnutí účtu, výběr fiat měny a~informace o~aplikaci.}
...
\caption{Multisig peněženky. Vlevo: import deskriptoru s~ikonou QR skeneru
a~pojmenováním. Uprostřed: seznam importovaných multisig peněženek. Vpravo:
detail peněženky 2-of-3 se zůstatkem, historií transakcí a~přístupem
k~seznamu PSBT.}
...
\caption{PSBT workflow. Zleva: seznam PSBT transakcí; detail PSBT se stavem
podpisů (1 ze~2); dialog signerů se stavy Signed~/~Pending a~příznakem
\emph{YOU}; PSBT s~dosaženým prahem (2 ze~2) připravené k~broadcastu.}
...
na backend a~zobrazí se i~ostatním účastníkům multisig peněženky. Cosigner
...
Po dosažení prahu~$M$ (obrázek~\ref{fig:screen-psbt-flow}, vpravo) backend
automaticky finalizuje PSBT (sestaví witness data se všemi podpisy ve
správném pořadí) a~na detailu PSBT se zobrazí tlačítko \emph{Broadcast
Transaction}. Po jeho stisknutí backend odešle výslednou transakci do
Bitcoinové sítě a~aplikace zobrazí potvrzení s~identifikátorem transakce
shodně jako u~singlesig odeslání.

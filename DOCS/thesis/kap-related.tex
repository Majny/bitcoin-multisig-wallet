...
\caption[Struktura PSBT pro multisig transakci]{Struktura PSBT pro
multisig transakci se dvěma vstupy a~dvěma výstupy. Globální mapa
obsahuje nepodepsanou transakci, mapy vstupů nesou data pro
podepisování a~mapa change výstupu obsahuje derivační cesty pro
identifikaci vlastního výstupu.}
...
\caption[Životní cyklus PSBT pro multisig transakci]{Životní cyklus
PSBT pro multisig transakci. Serverová služba zastává role Creator
a~Updater, shromažďuje podpisy a~aktualizuje parametry pro další
cosignery. Trezor Connect interně provádí finalizaci (Combiner,
...
\texttt{OP\_0} a~délky polí (přibližně 5~B), M~podpisů (každý maximálně
...
opcode). Implementace pro odhad poplatku zaokrouhluje dolů
(strictní BIP-141 vyžaduje stropní funkci, rozdíl je nejvýše 1~vB
na vstup, což je pro odhad poplatku zanedbatelné):
\[
  \text{vSize}_{\text{input}} \approx 41 + \left\lfloor \frac{5 + 73 \cdot M + 34 \cdot N}{4} \right\rfloor \text{~vB}
\]
Například pro schéma 2-of-3 vychází odhadovaná velikost jednoho vstupu
$41 + \lfloor (5 + 146 + 102)/4 \rfloor = 41 + 63 = 104$~vB.
...
Safe 7~\citep{TrezorSafe7} s~bezdrátovou Bluetooth konektivitou; starší
modely se připojují výhradně přes USB. Všechna zařízení podporují protokol Bitcoin včetně SegWit
a~multisig transakcí.

Důležité je rozlišovat kompatibilitu s~mobilními zařízeními. Pro deeplink
komunikaci zprostředkovanou aplikací Trezor Suite Mobile vyžadujeme modely
z~aktuální řady Safe, konkrétně Trezor Safe~3, Safe~5 nebo Safe~7. Starší
model \textbf{Trezor One} (první generace s~micro-USB konektorem) sice
s~aplikací Trezor Suite Mobile přes USB pracuje, ale deeplink rozhraní,
na~kterém naše aplikace závisí, je primárně určené pro řadu Safe, takže
pro naši aplikaci Trezor One použitelný není.
...
verze Trezor Suite Mobile navíc nabízí pouze omezenou podmnožinu funkcí
desktopové verze.
...
\textbf{Omezení}: Některé pokročilé funkce Nunchuku, jako jsou inheritance
planning nebo asistovaný multisig se službou Nunchuk, jsou dostupné pouze
v~rámci placeného předplatného. Tento model může být překážkou pro uživatele,
...
\noindent
Rozsah uvedených komponent (transportní vrstva, sada protobuf zpráv
odpovídající aktuálnímu firmware, stavový automat podepisování
a~multisig rozšíření) představuje samostatný implementační projekt
přesahující rámec bakalářské práce. Zdrojový kód aplikace Trezor Suite
je sice veřejně dostupný, ale je publikován pod licencí T-RSL (Trezor
Reference Source License)~\citep{TRSL}, která explicitně zakazuje
kopírování kódu do jiných projektů -- umožňuje pouze čtení za~účelem
studia. Implementace by proto musela vzniknout zcela od základu
na~základě veřejné dokumentace protokolu~\citep{TrezorFirmwareDocs}.
...
kontextu. Představuje však samostatný implementační projekt -- nutnost
implementace složitého protokolu zcela od základu (kvůli licenčním
omezením) -- který přesahuje rámec bakalářské práce. JavaScript SDK
je pro Android nepoužitelné. Delegování na Trezor Suite Mobile
prostřednictvím deeplinků je proto pragmatickou volbou.
...
PSBT je v~aplikaci využita jako standardní container pro koordinaci
podpisů mezi cosignery. I~v~rámci jednoho multisig workflow mezi
podpisy jednotlivých cosignerů typicky uplyne čas v~řádu hodin
až dní -- stav rozpracované transakce proto musí být kompletně
reprezentován jediným formátem se~všemi metadaty potřebnými
pro další podpis (utracený výstup, witness skript, derivační cesty
cosignerů, dosavadní dílčí podpisy). PSBT je pro tento účel přímo
navržen a~díky volbě standardu nezavádí aplikace proprietární
formát.
...
a~sbírá dílčí podpisy. Po dosažení prahu~M iniciuje uživatel broadcast
v~klientské aplikaci a~backend transakci finalizuje a~odešle do sítě.

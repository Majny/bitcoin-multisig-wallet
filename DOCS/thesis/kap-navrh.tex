...
Gateway přes HTTPS a~s~hardwarovou peněženkou Trezor přes deeplink protokol
...
\item \textbf{Směrování požadavků} -- na základě prefixu URL cesty gateway
  ...
  \texttt{/api/\allowbreak wallet} je směrován na \texttt{wallet-\allowbreak registry}
  (port~8082), prefix \texttt{/api/\allowbreak psbt} na \texttt{psbt-\allowbreak service}
  (port~8085) a~podobně.
\item \textbf{Propagace identity} -- po úspěšné validaci tokenu gateway
  ...
  cílové službě v~HTTP hlavičce. Jednotlivé služby tak nemusí opakovaně
  validovat token a~mohou důvěřovat identitě předané gateway.
\end{itemize}
...
\begin{itemize}
\item \textbf{JSONB datový typ} -- \texttt{psbt-service} potřebuje ukládat
  parametry pro Trezor Connect, které mají proměnlivou strukturu (závisí na
  typu peněženky, počtu vstupů a~výstupů). Datový typ JSONB umožňuje ukládat
  tyto parametry jako strukturovaný JSON přímo v~relační tabulce, s~možností
...
ale nabízí slabší podporu JSONB a~méně pokročilé možnosti indexace
strukturovaných dat.
...
CoinGecko API~\cite{CoinGeckoAPI}. CoinGecko nabízí bezplatný tier s~dostatečným limitem požadavků
(30~volání za minutu), podporuje desítky fiat měn a~vyžaduje pouze
bezplatnou registraci s~Demo API klíčem. Alternativou by bylo
CoinMarketCap API, které rovněž vyžaduje registraci, ale omezuje
bezplatné použití přísnějšími limity (20~volání denně).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{img/auth-flow.pdf}
...
Databáze \texttt{auth-service} obsahuje jedinou tabulku:
...
\caption{ER diagram datového modelu. Databáze auth-service obsahuje tabulku
zařízení, wallet-registry spravuje peněženky, cosignery a~adresy, psbt-service
uchovává PSBT transakce a~záznamy podpisů.}
...
\item[\texttt{psbts}] -- Hlavní tabulka PSBT transakcí. Obsahuje
  \texttt{psbt\_\allowbreak id} (UUID),
...
Uložení parametrů pro Trezor Connect ve formátu JSONB jsme zvolili proto,
že jejich struktura závisí na konkrétní transakci -- počet vstupů a~výstupů
se liší, každý vstup má jinou derivační cestu a~částku. Relační reprezentace
by vyžadovala zbytečně složité schéma s~více vazebnými tabulkami.
...
\item \textbf{Broadcast} -- Po dosažení prahu poslední podepisující Trezor
  vrátí kompletní \texttt{serializedTx} a~backend ji propaguje do sítě.
\end{enumerate}
...
prahu podpisů backend finalizuje PSBT a~odešle transakci do Bitcoinové sítě.}
...
\item[Detail transakce] -- Rozklikem libovolné položky ze seznamu
  transakcí na dashboardu. Zobrazuje \texttt{txid}, status, poplatek
  a~seznam vstupů a~výstupů s~částkami. Adresy patřící aktuální peněžence
  jsou označeny štítkem \emph{YOURS}. Kliknutím na vstup lze otevřít
  předchozí transakci, která daný UTXO vytvořila, a~procházet tak historii
  až do hloubky~pěti úrovní; na každé úrovni je štítkem \emph{FROM HERE}
  označen výstup, který navazuje na proklikaný vstup z~úrovně předchozí.
  Tato funkce slouží uživateli k~forenznímu ověření původu prostředků
  bez nutnosti přepínat do externího prohlížeče blockchainu.
...
\item[Detail transakce] -- Zobrazuje kompletní informace o~transakci:
  identifikátor, stav potvrzení, výšku bloku, časové razítko, poplatek
  a~sazbu poplatku v~sat/vB. Pro každý vstup a~výstup zobrazuje adresu,
  hodnotu a~příznak, zda adresa patří aktuální peněžence. Prokliknutím
  na vstup se otevře detail předchozí transakce -- tento rekurzivní průchod
  řetězcem transakcí je omezen na pět úrovní, aby nedošlo k~nadměrnému
  zatížení blockchainového API.
...
\item[PSBT detail] -- Detail konkrétní PSBT transakce se souhrnem částky,
  ...
  u~každého cosignera je vidět, zda podepsal, a~v~dialogu signerů lze cosignerům
  přiřadit pojmenování (například ,,Alicin klíč``), které se uloží na backend
  a~zobrazí se i~ostatním účastníkům. Pokud transakce čeká na podpis
  aktuálního cosignera, zobrazí se tlačítko pro podepsání.
...
\item \texttt{PSBT\_IN\_NON\_WITNESS\_UTXO} -- celá předchozí transakce v~raw
  formátu. Firmware Trezor od verze~2.4 vyžaduje tento záznam i~pro nativní
  SegWit vstupy, aby mohl ověřit částky vstupů a~zobrazit správný poplatek.

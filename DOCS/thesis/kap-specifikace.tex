...
\begin{description}
  ...
  \item[Hlavní scénář:] \hfill
    \begin{enumerate}
      ...
      \item Uživatel zadá částku buď v~BTC, nebo ve zvolené fiat měně
            (CZK, USD nebo EUR; konkrétní měna se volí v~nastavení).
            Přepínač jednotky vedle pole umožňuje mezi BTC a~fiat přepínat;
            aplikace pod polem vždy zobrazuje ekvivalent ve~druhé jednotce.
            Sats (tedy skutečná částka utracená v~transakci) se počítají
            z~BTC ekvivalentu: je-li vstup ve~fiat, aplikace jej nejprve
            převede přes aktuální kurz získaný ze~služby \texttt{price-service}.
...
\begin{description}
  ...
  \item[NF3 -- Důvěryhodný síťový kanál:]
    ...
    LAN nebo ve~VPN (ve~vývojovém nasazení autor používá Tailscale,
    které zajišťuje šifrování na~síťové vrstvě mezi zařízeními).
    TLS na~aplikační vrstvě není v~rámci této práce řešeno.
\end{description}
